import { loadConfig, getDefaultConfig, validateConfig, loadConfigFromPath, saveConfig } from '../config-loader';
...
describe('config-loader', () => {
  ...
  describe('loadConfig()', () => {
    ...
    it('loads default config when no config file exists', async () => {
      mockFs.access.mockRejectedValueOnce({ code: 'ENOENT' } as any);

      const config = await loadConfig();
      expect(config).toEqual(getDefaultConfig());
      expect(mockFs.access).toHaveBeenCalledWith(expect.stringContaining('.framework/config'));
