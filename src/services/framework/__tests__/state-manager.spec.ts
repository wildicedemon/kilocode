...
describe('state-manager', () => {
  ...
  describe('updateTaskStatus()', () => {
    ...
    it('throws if task not found', async () => {
      await expect(manager.updateTaskStatus('planning' as SdlcPhaseName, 'nonexistent', 'completed' as TaskStatus))
  ...
  describe('createCheckpoint()', () => {
    it('creates checkpoint and increments count', async () => {
      ...
      expect(mockFs.writeFile).toHaveBeenNthCalledWith(2, expect.stringContaining('checkpoints/checkpoint-0001.json'), expect.any(String));
    });
  });
  ...
  describe('listCheckpoints()', () => {
    it('lists checkpoints', async () => {
      ...
      mockFs.readdir.mockResolvedValueOnce(['checkpoint-0001.json']);
