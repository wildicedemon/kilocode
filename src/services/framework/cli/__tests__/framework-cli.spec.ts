...
describe('FrameworkCLI', () => {
  ...
  it('handles --version', async () => {
    ...
    mockProcess.mockValue(['node', 'cli.js', '--version']);
  ...
  describe('parseArgs()', () => {
    it('parses positional command', () => {
      const parsed = cli['parseArgs'](['start', 'arg1']);
      expect(parsed.command).toBe('start');
      expect(parsed._).toEqual(['arg1']);
    });

    it('parses long options', () => {
      const parsed = cli['parseArgs'](['--verbose', '--config', 'config.yaml']);
    ...
    it('parses --no-color', () => {
      const parsed = cli['parseArgs'](['--no-color']);
