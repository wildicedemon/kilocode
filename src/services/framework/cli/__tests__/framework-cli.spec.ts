import { FrameworkCLI, createCLI, main, parseArgs } from "../index"
...
describe("FrameworkCLI", () => {
	...
	it("handles --version", async () => {
		const mockProcess = require("process") as { argv: string[] }
		mockProcess.argv = ["node", "cli.js", "--version"]
	...
	describe("parseArgs()", () => {
		it("parses positional command", () => {
			const parsed = parseArgs(["start", "arg1"])
			expect(parsed.command).toBe("start")
			expect(parsed._).toEqual(["arg1"])
		})

		it("parses long options", () => {
			const parsed = parseArgs(["--verbose", "--config", "config.yaml"])
		...
		it("parses --no-color", () => {
			const parsed = parseArgs(["--no-color"])
